...
export const deletePost = (req,res) =>{
    ...
    const q  = 'DELETE FROM posts WHERE `id` = ?; UPDATE posts SET `id` = `id`-1 where `id` > ?; set @nextId := (select IFNULL(max(id),0)+1 from blog.posts); set @sql := CONCAT("alter table Blogmaze_site.posts AUTO_INCREMENT= " , @nextId); PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;' ; 
